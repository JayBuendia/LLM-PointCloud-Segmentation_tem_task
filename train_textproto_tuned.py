...
def build_optimizer(model, args):
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if name.startswith("segmentor.encoder.encoder_model"):
    ...
    groups = []
    if backbone_params:
        groups.append({"params": backbone_params, "lr": args.backbone_lr, "name": "pretrained_backbone"})
    if adapter_params:
...
def main():
    ...
    freeze_prefixes = [x.strip() for x in args.freeze_backbone_prefixes.split(",") if x.strip()]
    frozen_params, backbone_total_params = apply_partial_freeze(model, freeze_prefixes)
